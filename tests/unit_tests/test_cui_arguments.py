...
class TestFabFile:
    ...
    def test_default(self, fs):
        ...
        filename = parser.parse_fabfile_only([])
        assert isinstance(filename, Path)
        assert filename.name == "FabFile"
    ...
    def test_user(self, fs):
        """Check user specified argument."""

        fs.create_file("myfile")

        parser = FabArgumentParser()
        filename = parser.parse_fabfile_only(["--file", "myfile"])
        assert isinstance(filename, Path)
        assert filename.name == "myfile"

    def test_nonexistent_user(self, fs, capsys):
    ...
    def test_no_help(self):
        ...
        parser = FabArgumentParser()
        filename = parser.parse_fabfile_only(["--help"])
        assert filename is None
    ...
    def test_no_exit(self):
        ...
        parser = FabArgumentParser()
        filename = parser.parse_fabfile_only(["--file"])
        assert filename is None
